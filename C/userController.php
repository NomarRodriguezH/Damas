<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
		    ...
		    if($this->verificarDatos("[a-zA-Z0-9]{2,20}",$usuario)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
		    ...
		    if($this->verificarDatos("[a-zA-Z0-9$@.-]{2,100}",$clave1) || $this->verificarDatos("[a-zA-Z0-9$@.-]{7,100}",$clave2)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
		...
		public function registrarDamaControlador(){
		    ...
		    $clave1=$this->limpiarCadena($_POST['clave']);
		    ...
		    # Verificando campos obligatorios #
		    if($nombre=="" || $tipo=="" || $email=="" || $usuario=="" || $clave2=="" ||  $clave1=="" || $edad=="" || $medidas=="" || $estatura=="" || $descripcion==""){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No has llenado todos los campos que son obligatorios",
		    ...
		    if($email!=""){
				if(filter_var($email, FILTER_VALIDATE_EMAIL)){
					$check_email=$this->ejecutarConsulta("SELECT correo FROM damas WHERE correo='$email'");
					if($check_email->rowCount()>0){
						$alerta=[
							"tipo"=>"simple",
							"titulo"=>"Ocurrió un error inesperado",
            ...
            # Verificando usuario #
		    $check_usuario=$this->ejecutarConsulta("SELECT username FROM damas WHERE username='$usuario'");
		    if($check_usuario->rowCount()>0){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
		    ...
		    $usuario_datos_reg=[
				[
					"campo_nombre"=>"nombre", //Campo en BD
					"campo_marcador"=>":Nombre",
					"campo_valor"=>$nombre
				],
				[
				...
				[
					"campo_nombre"=>"damas_precios",
					"campo_marcador"=>":damas_precios",
					"campo_valor"=>$uno
				],
				[
					"campo_nombre"=>"damas_tipoServicio",
					"campo_marcador"=>":damas_tipoServicio",
					"campo_valor"=>$uno
				],
				[
					"campo_nombre"=>"damas_zonaServicio",
					"campo_marcador"=>":damas_zonaServicio",
					"campo_valor"=>$uno
				],
				[
					"campo_nombre"=>"damas_fisico",
					"campo_marcador"=>":damas_fisico",
					"campo_valor"=>$uno
				],      
				[
					"campo_nombre"=>"creado",
			...
			$registrar_usuario=$this->guardarDatos("damas",$usuario_datos_reg);

			if($registrar_usuario->rowCount()==1){
				$alerta=[
					"tipo"=>"limpiar",
					"titulo"=>"Usuario registrado",
					"texto"=>"La dama ".$usuario." se registro con exito",
					"icono"=>"success"
				];
			}else{
				...
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No se pudo registrar a la dama, por favor intente nuevamente",
					"icono"=>"error"
				];
			}
		...
		public function registrarClientaControlador(){
		    ...
		    # Verificando campos obligatorios #
		    if($nombre=="" || $tipo=="" || $email=="" || $usuario=="" || $clave2=="" ||  $clave1==""){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No has llenado todos los campos que son obligatorios",
		    ...
		    if($this->verificarDatos("[a-zA-Z0-9$@.-]{4,80}",$clave1) || $this->verificarDatos("[a-zA-Z0-9$@.-]{4,80}",$clave2)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"Las CLAVES no coinciden con el formato solicitado",
		    ...
		    if($email!=""){
				if(filter_var($email, FILTER_VALIDATE_EMAIL)){
					$check_email=$this->ejecutarConsulta("SELECT correo FROM clienta WHERE correo='$email'");
					if($check_email->rowCount()>0){
						$alerta=[
							"tipo"=>"simple",
							"titulo"=>"Ocurrió un error inesperado",
            ...
            # Verificando usuario #
		    $check_usuario=$this->ejecutarConsulta("SELECT username FROM clienta WHERE username='$usuario'");
		    if($check_usuario->rowCount()>0){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
		    ...
		    $usuario_datos_reg=[
				...
				[
					"campo_nombre"=>"apellido", //Campo en BD
					"campo_marcador"=>":Apellido",
					"campo_valor"=>$apellido
				],
				[
			...
			$registrar_usuario=$this->guardarDatos("clienta",$usuario_datos_reg);
